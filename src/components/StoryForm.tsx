...
export default function StoryForm({ story, onSubmit, onCancel }: StoryFormProps) {
  ...
  return (
    <div className="modal-overlay" onClick={onCancel} data-testid="story-form">
      <div
      ...
      >
        ...
        <div className="mb-4">
          ...
          <input data-testid="story-name" className={inputCls} placeholder="np. Logowanie użytkownika" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
        </div>

        <div className="mb-4">
          <label className={`form-label ${isDark ? "text-slate-400" : "text-slate-500"}`}>Opis</label>
          <textarea data-testid="story-description" className={`${inputCls} min-h-20 resize-y`} placeholder="Opis historyjki…" value={description} onChange={(e) => setDescription(e.target.value)} />
        </div>

        <div className="flex gap-3 mb-5">
          <div className="flex-1">
            <label className={`form-label ${isDark ? "text-slate-400" : "text-slate-500"}`}>Priorytet</label>
            <select data-testid="story-priority" className={inputCls} value={priority} onChange={(e) => setPriority(e.target.value as Priority)}>
          ...
          <div className="flex-1">
            ...
            <select data-testid="story-status" className={inputCls} value={status} onChange={(e) => setStatus(e.target.value as StoryStatus)}>
        ...
        <div className="flex justify-end gap-2.5">
          <button data-testid="story-cancel" className={`px-5 py-2.5 rounded-xl text-sm border transition-colors ${isDark ? "border-white/10 text-slate-400 hover:bg-white/5" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`} onClick={onCancel}>Anuluj</button>
          <button data-testid="story-submit" className={`btn-primary ${!name.trim() ? "opacity-50 cursor-not-allowed" : ""}`} onClick={handleSubmit} disabled={!name.trim()}>
